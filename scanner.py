...
SECONDS_BEFORE_RECORD_EXPIRATION = 240  # how many seconds a stream should stay in the db
...
def populate_streamers(client_id, client_secret):
    ...
    while True:
        ...
        # filter out streams with our desired count and insert into db
        ...
        if streams_found:
            ...
            db_ready_rows = [(stream['id'], stream['game_name'].lower(), int(time.time()), json.dumps(stream)) for stream in streams_found]
            conn.executemany("INSERT INTO streams VALUES(?, ?, ?, ?)", db_ready_rows)
